...
for folder in 'SuperEnhancers/Constituent_Enhs' 'SuperEnhancers_27ac/Constituent_Enhs' ; do
  ...
  for TF in $TFS ; do
    ...
    for MOTIF in $MOTIFS; do
      #ruby score_distribution.rb TSRE/mm10/$folder/${CL}.te.const.txt.bed gtrd/adaptive_quality/${TF}.bed $MOTIF  0.001  chipseq_enhancer_bed/$folder/${CL}.te.const.${MOTIF}.txt  raw_scores/$folder/${CL}.te.const.${MOTIF}.txt  score_stats/$folder/${CL}.te.const.${MOTIF}.txt TE
      #ruby score_distribution.rb TSRE/mm10/$folder/${CL}.se.const.txt.bed gtrd/adaptive_quality/${TF}.bed $MOTIF  0.001  chipseq_enhancer_bed/$folder/${CL}.se.const.${MOTIF}.txt  raw_scores/$folder/${CL}.se.const.${MOTIF}.txt  score_stats/$folder/${CL}.se.const.${MOTIF}.txt SE
      ( \
