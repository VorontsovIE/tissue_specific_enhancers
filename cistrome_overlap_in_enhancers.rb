require 'fileutils'
FileUtils.mkdir_p 'cistrome_overlap_per_enhancer'

Dir.glob("gtrd/adaptive_quality/*.bed").each{|tf_fn|
  tf = File.basename(tf_fn, '.bed')
  FileUtils.mkdir_p "cistrome_overlap_per_enhancer/#{tf}"
  ...
  Dir.glob("TSRE/glued/SuperEnhancers_27ac/Constituent_Enhs/*.bed").each{|glued_enhancers_fn|
    ...
    output_fn = "cistrome_overlap_per_enhancer/#{tf}/#{tf}.#{enhancer}.bed"
    cmd = [
